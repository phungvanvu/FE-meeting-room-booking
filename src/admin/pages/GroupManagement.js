...
const GroupManagement = () => {
  ...
  const saveGroup = (newGroup) => {
    if (currentGroup) {
      setGroups(groups.map(group => (group.id === currentGroup.id ? newGroup : group)));
    } else {
  ...
  return (
    <div className="flex flex-col p-6 max-w-4xl mx-auto space-y-6 min-h-screen">
      <div className="flex items-center mb-4">
        ...
        <h2 className="text-center text-2xl font-bold flex-grow">Danh sách Nhóm </h2>
      </div>
      ...
      <table className="min-w-full bg-white border border-gray-300">
        ...
        <tbody>
          {groups
            .filter(group => group.name.toLowerCase().includes(searchTerm.toLowerCase())) // Lọc theo tên
            .map((group) => (
              <tr key={group.id} className="border-b hover:bg-gray-100 transition">
...
const GroupForm = ({ currentGroup, onSave, onCancel, locations }) => {
  ...
  const [division, setDivision] = useState(currentGroup ? currentGroup.division : "");
  const [location, setLocation] = useState(currentGroup ? currentGroup.location : "");

  const handleSubmit = (e) => {
    e.preventDefault();
    onSave({ id: currentGroup ? currentGroup.id : null, name, division, location });
  };

  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-30">
      <div className="bg-white p-6 rounded-lg shadow-lg" style={{ width: '400px' }}>
        <h2 className="text-xl font-bold mb-4">{currentGroup ? "Sửa Nhóm" : "Thêm Nhóm"}</h2>
